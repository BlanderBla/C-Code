...
/*
	...
	XXX ELIMINAR AL FINAL DE LA LISTA
...
void insertar_Final();
void insertar_Cualquier_Elemento();
void mostrar_lista();
void eliminar_Primer_Elemento();
void eliminar_Ultimo_Elemento();
void eliminar_Cualquier_Elemento();
...
void eliminar_Ultimo_Elemento(){
	...
	if(primero != NULL) //Comprueba si hay datos en la lista
	{
		if(nuevo->siguiente != NULL){
			while((nuevo->siguiente)->siguiente != NULL){
				nuevo = nuevo->siguiente;
			}
			free(nuevo->siguiente);
			nuevo->siguiente = NULL;
		}else{
			primero = NULL;
		}
...
//Eliminar el cualquier elemento de la lista
void eliminar_Cualquier_Elemento(){
	struct agenda *lista;
	lista = primero;
	
	char nombre2[20];
	printf("Nombre que desea eliminar: ");
    scanf("%s",&nombre2);
	//preguntar si la lista no esta vacia
	if(lista != NULL){
		struct agenda *aux_borrar;
		struct agenda *anterior = NULL;
			
		aux_borrar = lista;
	   //if(strcmp(clave,(productos+i)->clave)==0)
	   printf("valor de comparacion %d",strcmp(nombre2,aux_borrar->nombre));
		//Recorrer la lista y una vez que lo encuentra aux_borrar es Nodo que estamos buscando
		while((aux_borrar != NULL) &&(strcmp(nombre2,aux_borrar->nombre)!=0)){
			anterior = aux_borrar;
		...
		}else if(anterior==NULL){	//EL primer elemento que vamos a eliminar
			lista = lista->siguiente;
			delete aux_borrar;
			primero=lista;
		}else{	// EL elemento que no esta en la primera lista, pero no es el primer nodo
