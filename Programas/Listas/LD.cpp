...
void listacirculares(int *);
void mostrarListaSimEnl(int);

main(){
	int opc=0, total=0, contador=0;
	do{
		menu();
		scanf("%i",&opc);
		switch(opc){
			...
			case 4:
				system("cls");
					listacirculares(&contador);
				system("cls");	
				break;	
			case 5:
				system("cls");
					mostrarListaSimEnl(contador);
				system("cls");
				break;
			case 6:
				printf("Salir");
...
void mostrarLastToFirst(int total){
	...
	while (auxiliar!=NULL){
		...
		i--;	//EL DECREMENTO ES PARA VERIFICAR QUE HAYA DATOS,POR ESA RAZON ENTRO EN ESTE WHILE
	}
	//	VERIFICA QUE LA LISTA NO ESTE VACIA CON EL 	'i', POR ESO VA DESPUES DEL WHILE YA QUE SI NO ENTRO EN EL WHILE LA LISTA ESTA VACIA
...
void listacirculares(int *contador){
	
	struct agenCirc *nuevo;
	struct agenCirc *auxiliar; 
	nuevo = (struct agenCirc *) malloc(sizeof(struct agenCirc));
	if (nuevo==NULL){
...
void mostrarListaSimEnl(int contador){
    int i=0;
	struct agenCirc *auxiliar; 
    auxiliar = first;
    printf("\n\tMostrar lista simplemente enlazada circular\n");
